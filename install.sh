...
DEB="$SCRIPT_DIR/tuxgenie_3.6.0_all.deb"
...
install_tuxgenie() {
    ...
    if [ -n "$OLD_VER" ]; then
        ...
        echo "  Upgrading to v3.6.0 — your API key and settings will be kept."
    else
        echo "  Fresh install — installing TuxGenie v3.6.0."
    fi
    echo ""
    ...
    if sudo dpkg -i "$DEB"; then
        echo ""
        if [ -n "$OLD_VER" ]; then
            echo "  ✓ Upgraded from v$OLD_VER  →  v3.6.0 successfully!"
        else
            echo "  ✓ TuxGenie v3.6.0 installed successfully!"
        fi
        echo ""
        echo "  How to use:"
...
if [ -t 1 ]; then
    ...
    sudo dpkg -i "$DEB" && zenity --info --text="TuxGenie v3.6.0 installed!\nOpen a terminal and type: tuxgenie" 2>/dev/null
fi
